"""Tests for ckanext.{{ project_shortname }}.view.
...
def test_page(app: CKANTestApp):
    ...
    resp = app.get(tk.url_for("{{project_shortname}}.page"))
...
def test_page_redirect(app: CKANTestApp):
    ...
    resp = app.get(
        tk.url_for("{{project_shortname}}.page_redirect"),
    ...
    expected = tk.url_for("{{project_shortname}}.page", _external=True)
...
class TestComplex:
    def test_get(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.get(tk.url_for("{{project_shortname}}.complex", word=word))
    ...
    def test_post_invalid(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.post(tk.url_for("{{project_shortname}}.complex", word=word))
        page = bs4.BeautifulSoup(resp.body)
    ...
    def test_post_valid(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.post(
            tk.url_for("{{project_shortname}}.complex", word=word),
            data={"left": "1", "right": "2"},
        )
        page = bs4.BeautifulSoup(resp.body)
